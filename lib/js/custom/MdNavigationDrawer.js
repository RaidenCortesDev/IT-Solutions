...
export class MdNavigationDrawer extends HTMLElement {
	...
	getContent() {
		return /* HTML */ `
			...
			<style>
				...
				a:active {
					/* inactive pressed label, shape */
					background-position: center;
					background-image: radial-gradient(circle, var(--md-riple-color) 1%, transparent 1%);
					background-size: 100%;
				...
				a.active:active {
					/* active pressed label, shape */
					color: var(--md-sys-color-on-secondary-container);
				}

				a:active::before {
					/* inactive pressed state layer */
					background-color: var(--md-sys-color-on-surface);
					opacity: var(--md-sys-state-pressed-state-layer-opacity);
				}

				a.active:active::before {
					/* active pressed state layer */
					background-color: var(--md-sys-color-on-secondary-container);
				}

				a:active span {
					/* inactive pressed icon */
					color: var(--md-sys-color-on-surface);
				}
			...
			<div id="scrim"></div>
			<nav class="large-end"><div></div></nav>
		`;
	}
	...
	constructor() {
		...
		this._div = querySelector(shadow, "nav>div");
