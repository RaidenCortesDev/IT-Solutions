...
export class MdOverflowMenu extends HTMLElement {
	...
	alterna(toggleButton) {
		...
		const right = innerWidth - (toggleButton.offsetLeft + toggleButton.offsetWidth) - 3;
	...
	clicCierra(evt) {
		...
		if (this.classList.contains("open") && this._toggleButton !== null && target instanceof HTMLElement && !this._toggleButton.contains(target)) {
			this.cierra();
		}
	}
}
